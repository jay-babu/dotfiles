...
For disaster recovery / dotfiles / "what should we track in `.hermes`?" questions, load `references/disaster-recovery-dotfiles.md`. Default recommendation: track stable config and skills, encrypt `.env`/`auth.json`/webhook secrets, back up mutable state (`state.db`, `sessions/`) separately, and inspect host-level systemd/reverse-proxy config plus local Hermes source patches.

For unattended AWS access for Hermes running outside AWS (especially when the user mentions AWS Organizations, IAM Identity Center, gamma/production accounts, AWS CLI profiles, or chezmoi tracking), load `references/aws-roles-anywhere.md`. Key lesson: IAM Identity Center is good for human/bootstrap access but not perpetual unattended refresh; prefer IAM Roles Anywhere with checksum-verified `aws_signing_helper`, tightly-scoped per-account roles, certificate subject/issuer trust-policy conditions, and chezmoi tracking only for non-secret config/scripts.
...
For webhook-triggered runs cross-delivered to Slack that post repeated top-level `⏳ Still working...` messages instead of one thread (e.g. PagerDuty incident automation), load `references/webhook-slack-threading.md`. Key pitfall: webhook delivery only forwards static `deliver_extra.thread_id`; without dynamic per-object Slack `thread_ts` mapping, heartbeats and final responses are separate top-level Slack messages.
