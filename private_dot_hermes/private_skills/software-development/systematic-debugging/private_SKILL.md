...
For Go/Bob related-field sorting with API expands, verify the parent query shape before fixing symptoms. `SelectThenLoad` performs separate batched relationship SELECTs and does not add joins to the parent/root query, so `ORDER BY related_table.column` requires join-based `Preload` or explicit join mods. Prefer recursive generated `Preload.ForExpandMap` helpers with deterministic `psql.PreloadAs(...)` aliases for normal FK expand paths; keep custom joins for computed/function-backed paths. When scanning SQL-function rows into concrete Go types, preserve legacy nullability handling such as `COALESCE(points, 0)` before `int64` scans. See `references/bob-preload-expand-sorting.md`.
