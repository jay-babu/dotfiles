...
- `references/sheets-email-correspondence-updates.md` — workflow for updating existing product/vendor tracker tabs from latest Gmail correspondence, including thread search/read, tab mapping, batched row writes, and read-back verification.
...
- When updating vendor/product tracker tabs from email correspondence, search Gmail broadly, read full threads rather than snippets, map each lead to all relevant product tabs, write concise status/MOQ/pricing/next-action rows, and verify representative tabs after the batch update. See `references/sheets-email-correspondence-updates.md`.
