...
For Transformity POSBackend PRs, see `references/posbackend-pr-workflow.md` for fresh-clone/submodule setup, Gradle verification commands, Testcontainers Docker-subnet cleanup, REST PR creation when `gh` is absent, and POSBackend-specific CI interpretation notes. For Pulumi/IaC review fixes where a dependent resource targets an alarm created by a helper, see `references/pulumi-alarm-resource-targets.md`; prefer passing the created alarm resource's `.name` over hardcoding the generated alarm name string.

For Transformity POS frontend PRs, see `references/transformity-frontend-pr-workflow.md` for canonical worktree placement, Node 22/mise validation, frontend artifact hygiene, focused table sort-column-id fixes, and the workflow for superseding backend workaround PRs with frontend fixes. For Statsig bootstrap/feature-flag startup fixes, see `references/statsig-bootstrap-modern-format.md`; prefer the modern bootstrap payload shape (`user` field plus gate evaluation objects) over switching to legacy bootstrap. For npm-to-Bun/Vite migration follow-ups, see `references/transformity-frontend-bun-vite-finalization.md` for `bunx --bun vite` script alignment, Amplify cache guardrails, Playwright/Stably `npx` exceptions, and validation/CI interpretation notes. For Stably incident PRs whose failure is sales item search/result timing, see `webhook-subscriptions/references/stably-posfrontend-sales-search-timeout.md` for the narrow helper-timeout pattern and how to word local-vs-CI verification.
...
For Zeus/Kurama PRs that touch HTMX templates, web assets, or model SQL, see `references/zeus-kurama-generated-prs.md` for required `go generate` commands, targeted validation, generated-file staging expectations, and how to report broad Atlas/Testcontainers integration failures. When rebasing Kurama branches with TypeSpec/generated-file conflicts, see `references/zeus-kurama-rebase-generated-conflicts.md`: resolve TypeSpec source first, preserve new base model fields, regenerate tracked outputs, and align handlers/audit/permission maps to the generated operation names before continuing the rebase.
...
For cross-repo changes, keep branch names aligned where practical (for example the same `fix/<topic>` in each repo), create one PR per repository, and report them back as a compact list grouped by repo. Before the final user note, run `git status --short` in each changed worktree and include honest validation notes: codegen/formatting that passed, tests that failed or were blocked, and the known cause when available. Do not imply the batch is fully green if any repo's local tests failed or were environment-blocked. If a prerequisite PR was already merged and a reviewer/user later corrects the mapping or rollout shape, create a new follow-up branch/PR from latest `origin/main` rather than force-pushing or amending the merged branch; then comment on the dependent PR with the new prerequisite links.

For permission rename rollouts, especially Zeus/Kurama Resource:Action enforcement, prepare prerequisite write/backfill PRs before treating the backend enforcement PR as ready: a `pos-db` Casbin backfill migration and a frontend `permissionDualWrite.ts` update. Include transient branch-generated permission names (plural interfaces, `Archive` actions, split subresource namespaces) as well as old production names. See `references/permission-rename-rollout-prereqs.md`.

When a reviewer/user says part of a PR must deploy first to avoid a breaking change, split by deployment order rather than leaving the behavior in the current PR. Put the prerequisite write/backfill/dual-write behavior on a fresh branch from the base branch, remove it from the original PR, comment with the prerequisite PR link, and verify both PRs independently. After the prerequisite PR merges, rebase the original PR onto the updated base and treat any prior "remove/move prerequisite behavior out" cleanup commits as obsolete; skip/drop those commits rather than re-removing behavior now owned by `main`. Expect conflicts in shared helper/test files and resolve toward: base owns prerequisite write/dual-write/backfill behavior, original PR owns only the enforcement/read-path changes. See `references/deployment-order-pr-splitting.md`.
...
4. After committing and pushing fixes, resolve the specific review threads you addressed. Use the thread IDs from the PR review-thread API/GraphQL rather than assuming a comment is resolved because a commit exists. If review replies contain shell-sensitive text (backticks, apostrophes, SQL escapes), prefer a short Python `subprocess.run([...])` / `execute_code` call over interpolating GraphQL bodies through fish or bash; this avoids broken quoting while keeping tokens out of logs.
5. If a reviewer comment is wrong or intentionally covered elsewhere, reply with the concrete evidence (for example the applied backfill commit or rollout invariant) and resolve the thread when appropriate; do not add compatibility code just to appease an automated review if it would create a second source of truth.
6. Verify the result by querying unresolved review threads again and report the count (for example “GitHub reports 0 unresolved threads”).
...
- A PR can report `mergeable_state: dirty` after visible checks pass because the base branch already merged overlapping work. Fetch the PR's actual `base.sha`/`base.ref` from the GitHub API, then fetch/rebase against `FETCH_HEAD` or the exact base SHA rather than trusting a stale or broken local `origin/main` ref. In long incident runs, also compare the real remote default branch to the local tracking ref (`git ls-remote origin refs/heads/main` vs `git rev-parse origin/main`) and force-fetch the exact ref if they differ: `git fetch origin refs/heads/main:refs/remotes/origin/main`. If the conflict shows the functional change is already present on the base branch, close/comment the PR as a duplicate/no-op instead of resolving conflicts just to recreate the same change. For current `gh`, close-with-comment is two commands (`gh pr comment --body-file ...` then `gh pr close ...`); do not rely on an unsupported `gh pr close --comment-file` flag. When this happens during an incident workflow, use the superseding PR URL plus successful post-merge CI/Stably run as the final evidence marker before posting a deduped PagerDuty note. See `references/superseded-incident-prs.md` for the full pattern.
...
For GitHub Actions deployments that use protected environments and `concurrency.cancel-in-progress: false`, do not assume new commits will cancel old approval prompts. Also beware matrix workflows that run an unprotected environment (for example gamma) alongside an approval-protected environment (production): the workflow run can report `waiting` while gamma is still actively testing/deploying. A cleanup action such as `styfle/cancel-workflow-action` with `only_status: waiting` and `force_cancel: true` cancels the entire older workflow run, not just the production approval job; that can kill in-progress gamma tests, prevent app-watermark/cache post-steps, and force the next commit to repeat the full deploy. Inspect per-job states and cancellation timestamps before attributing the delay. Prefer separate workflow runs for approval-gated production, or cancellation logic that cannot terminate an active gamma job.
...
## Bulk PR close / stale-PR cleanup safety

When the user asks to close many PRs, treat it as a destructive bulk operation and make the selection criteria auditable before mutation:

1. Derive the exact repo, state, author/app, age cutoff, and any branch/title filters. Use live time for relative cutoffs like “older than a month”.
2. If the user asks to “send the list first”, “double check”, or otherwise implies review before action, list the candidate PR numbers, titles, authors, branches, creation dates, and URLs, then stop. Do not close until the user explicitly approves the listed set.
3. For bot/app cleanup, require more than a title match. Prefer both `author.login` (for example `app/dependabot`) and a bot-owned branch pattern (for example `dependabot/*`) before closing. This avoids closing human PRs with dependency-style titles.
4. For “my PRs” in Transformity repos, map the user to their GitHub login when known (Jay = `jay-babu`) and filter by author rather than by branch naming alone.
5. When also deleting branches, first read each PR’s `headRefName` and `headRepositoryOwner`; only delete branches in the target repo that correspond exactly to the closed PR heads. Verify with `gh api repos/<owner>/<repo>/git/ref/heads/<url-encoded-branch>` or `git ls-remote --heads` before/after deletion. Report branches that are already missing as “nothing left to delete,” not as a failure.
6. After closing/deleting, re-query the same selection criteria and report the remaining count plus the exact PRs/branches affected.
